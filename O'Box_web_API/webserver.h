...
class Webserver {
	...
	private:
		...
		int SentUpgradeAvailResponse(struct MHD_Connection *conn);
		...
		int SendDeviceID_sceneaction(struct MHD_Connection *conn);
		int SendDeviceInitialize(struct MHD_Connection *conn);
		int SendDeviceRename(struct MHD_Connection *conn);
		int SendPollResponseEverySeconds();
